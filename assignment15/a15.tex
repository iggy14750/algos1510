...
\prob{26}
Now the input is a matrix $R_{t, p}$ which represents the number of users who requested page $p$ at time $t$.

Let's consider a similar pruning rule to before:
\begin{enumerate}
    \item Suppose there are two nodes which are at the same level, 
\end{enumerate}
