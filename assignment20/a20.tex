...
\prob{16}
This is so easy you should fucking kill yourself, Pruhs.
\begin{lstlisting}
DominatingSet(G)
    C = VertexCover(G)
    for vertex v in C do
        for vertex u adjacent to v do
            if u is NOT adjacent to any other vertex in C then
                skip v
        C = C - v
\end{lstlisting}
\end{document}
